...
public class ModItemModelProvider extends ItemModelProvider {
    public ModItemModelProvider(PackOutput output, ExistingFileHelper existingFileHelper) {
        super(output, BeyondEarth.MODID, existingFileHelper);
    }
    ...
    protected void registerModels() {
        ...
        blockItemModel(ItemsRegistry.URANIUM_BLOCK_ITEM);
        blockItemModel(ItemsRegistry.RAW_URANIUM_BLOCK_ITEM);
        ...
        blockItemModel(ItemsRegistry.MERCURY_URANIUM_ORE_ITEM);
        ...
        blockItemModel(ItemsRegistry.GLACIO_URANIUM_ORE_ITEM);
    }

    private ItemModelBuilder handheldModel(RegistryObject<?> item) {
        return getBuilder(item.getId().getPath()).parent(new ModelFile.UncheckedModelFile("item/handheld")).texture("layer0", new ResourceLocation(BeyondEarth.MODID, "item/" + item.getId().getPath()));
    }

    private ItemModelBuilder blockItemModel(RegistryObject<?> item) {
        return getBuilder(item.getId().getPath()).parent(new ModelFile.UncheckedModelFile(new ResourceLocation(BeyondEarth.MODID, "block/" + item.getId().getPath())));
    }
    ...
    private ItemModelBuilder armorItemModel(RegistryObject<?> item) {
        return getBuilder(item.getId().getPath()).parent(new ModelFile.UncheckedModelFile("item/generated")).texture("layer0", new ResourceLocation(BeyondEarth.MODID, "item/armor/" + item.getId().getPath()));
    }

    private ItemModelBuilder templateModel(RegistryObject<?> item, String template, String texturePath, String textureType) {
        return getBuilder(item.getId().getPath()).parent(new ModelFile.UncheckedModelFile(new ResourceLocation(BeyondEarth.MODID, "template/" + template))).texture(textureType, new ResourceLocation(BeyondEarth.MODID, texturePath + "/" + item.getId().getPath()));
    }
}
